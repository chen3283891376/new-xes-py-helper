...
export class PythonProcessManager {
    ...
    async startProcess(filePath: string, workDir: string): Promise<void> {
        ...
        if (this.pythonProcess.stdout) {
            // @ts-ignore
            readStream(this.pythonProcess.stdout.getReader(), this.stdoutCallback);
        }
        if (this.pythonProcess.stderr) {
            // @ts-ignore
            readStream(this.pythonProcess.stderr.getReader(), this.stderrCallback);
        }
    ...
    sendInput(data: string): void {
        if (this.pythonProcess && this.pythonProcess.stdin) {
            (this.pythonProcess.stdin as unknown as Writable).write(new TextEncoder().encode(data));
    ...
    async installPackage(packageName: string): Promise<{ exitCode: number; output: string }> {
        const proc = spawn({
            cmd: [
